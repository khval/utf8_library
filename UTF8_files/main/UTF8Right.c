...
*
*   SYNOPSIS
*       unsigned char * UTF8Right( unsigned char *UTF8, int _len, ULONG flags )
*
*   FUNCTION
*
*   INPUTS
...
unsigned char * _UTF8_UTF8Right(struct UTF8IFace *Self,
		unsigned char *UTF8, int _len, ULONG flags )
{
	...
	for (n=0;n<_start;n++)
	{
		value = _UTF8_UTF8GetGlyph( Self, UTF8 + pos, &len );
		pos += len;
	...
	for (n=0;n<_len;n++)
	{
		value = _UTF8_UTF8GetGlyph( Self, UTF8 + pos + size, &len );
		size += len;
		if (value == 0) return NULL;
	}
